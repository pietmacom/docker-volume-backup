...
echo "${_cronScheduleNormalized} /root/backup.sh 2>&1 | tee /var/log/docker-volume-backup.log > /proc/1/fd/1 2>&1 || /root/backup-notify.sh /var/log/docker-volume-backup.log" > /var/spool/cron/crontabs/root # Add our cron entry, and direct stdout & stderr to Docker commands stdout
